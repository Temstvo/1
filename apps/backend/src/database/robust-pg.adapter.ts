...
/**
 * Supavisor/pgbouncer может оборвать сокет в окне между `new Client()`
 * и навешиванием pool-листенера — тогда unhandled 'error' роняет процесс.
 * Noop-хендлер в конструкторе закрывает окно; pool всё равно получает
 * ошибки через свой idleListener -> pool.on('error').
 */
...
function withSafeClient<T extends { Client?: any }>(config: T): T {
  if (!config.Client) config.Client = SafeClient;
  return config;
}
import {
  ColumnTypeEnum,
...
class RobustPgQueryable {
  ...
  protected async recreatePool(): Promise<void> {
    const old = this.pool;
    this.pool = new Pool(withSafeClient({ ...(this.poolConfig as any) }));
    this.pool.on('error', () => {});
    if (old) {
      old.end().catch(() => {});
    }
...
export class RobustPg {
  ...
  async connect(): Promise<SqlDriverAdapter> {
    this.pool = new Pool(withSafeClient({ ...(this.config as any) }));
    this.pool.on('error', () => {});
