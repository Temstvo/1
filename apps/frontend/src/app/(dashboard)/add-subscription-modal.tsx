...
export default function AddSubscriptionModal({ open, onClose, onServersAdded }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-[#1a1a20] border border-[#2a2a32] rounded-2xl w-full max-w-md mx-4 overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between px-6 pt-6 pb-2">
          <h2 className="text-lg font-semibold text-white">
            {step === 'code'
              ? t('modal_title') || 'Add configuration'
              : t('modal_details_title') || 'Subscription details'}
          </h2>
          <button
            onClick={handleClose}
          ...
          {step === 'code' ? (
            ...
            <div className="space-y-4">
              <p className="text-sm text-[hsl(var(--muted-foreground))]">
                {t('modal_code_desc') ||
                  'Enter the 3-character activation code from your subscription'}
              </p>
              <div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-0 border border-[hsl(var(--border))] rounded-xl overflow-hidden">
                <div className="flex items-center justify-between px-4 py-3 border-b border-[hsl(var(--border))]">
                  <div>
                    <div className="text-sm text-white">
                      {t('modal_hide_settings') || 'Hide server settings'}
                    </div>
                    <div className="text-xs text-[hsl(var(--muted-foreground))]">
                      {t('modal_hide_settings_desc') ||
                        "You won't be able to edit server settings in this subscription"}
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between px-4 py-3 border-b border-[hsl(var(--border))]">
                  <div>
                    <div className="text-sm text-white">
                      {t('modal_encrypted') || 'Encrypted subscription'}
                    </div>
                    <div className="text-xs text-[hsl(var(--muted-foreground))]">
                      {t('modal_encrypted_desc') || 'The link you are adding is encrypted'}
                    </div>
                  </div>
                  <span className="text-xs text-[hsl(var(--muted-foreground))]">
                    {t('modal_off') || 'Off'}
                ...
                <div className="flex items-center justify-between px-4 py-3">
                  <div>
                    <div className="text-sm text-white">
                      {t('modal_unsafe') || 'Allow unsafe connection'}
                    </div>
                    <div className="text-xs text-[hsl(var(--muted-foreground))]">
                      {t('modal_unsafe_desc') || 'Allow unencrypted connections'}
                    </div>
                  </div>
                  <span className="text-xs text-[hsl(var(--muted-foreground))]">
                    {t('modal_off') || 'Off'}
