  ...
  return (
    <div className="flex h-full flex-col md:flex-row">
      ...
      <div className="md:hidden flex border-b border-[var(--border)]">
        ...
        <button onClick={() => setMobileView('detail')} className={`flex-1 py-3 text-sm font-medium text-center transition-colors ${mobileView === 'detail' ? 'text-[var(--primary)] border-b-2 border-[var(--primary)]' : 'text-[var(--muted-foreground)]'}`}>
          {t('vpn_how_to_use')}
        </button>
      </div>
      ...
      <div className={`${mobileView === 'detail' ? 'hidden md:flex' : 'flex'} w-full md:w-[320px] lg:w-[420px] border-r border-[var(--border)] flex-col shrink-0`}>
        <div className="p-4">
          ...
          {source === 'github' && (
            <div className="mb-3 px-3 py-2 rounded-lg bg-[var(--muted)] border border-[var(--border)]">
              <p className="text-xs text-[var(--muted-foreground)]">
                <span className="font-medium text-[var(--foreground)]">igareck/vpn-configs-for-russia</span> — {configs.length} {configs.length === 1 ? 'config' : 'configs'}
              </p>
            </div>
          )}
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-4">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-10 gap-2">
              ...
              {loadingMsg && <p className="text-xs text-[var(--muted-foreground)]">{loadingMsg}...</p>}
            </div>
            ...
            {showFullUri === selectedConfig.id && fullUri && (
              <div className="bg-[var(--muted)] rounded-xl p-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-medium text-[var(--muted-foreground)] uppercase">{t('vpn_config_uri')}</span>
                ...
                <pre className="text-xs text-[var(--muted-foreground)] break-all whitespace-pre-wrap font-mono max-h-40 overflow-y-auto">{fullUri}</pre>
              </div>
            )}

            <div className="bg-[var(--muted)] rounded-xl p-4 space-y-2">
              <h3 className="text-xs font-medium text-[var(--muted-foreground)] uppercase mb-3">{t('vpn_how_to_use')}</h3>
