...
import { VpnAutoConnect } from '@/components/vpn/vpn-auto-connect';
...
export default function VpnPage() {
  ...
  const flagUrl = (code: string) => `https://flagcdn.com/w80/${(code || 'un').toLowerCase()}.png`;
    ...
    <div className="flex h-full flex-col md:flex-row">
      {!isTauri && (
        <div className="bg-gradient-to-r from-purple-600/10 to-blue-600/10 border-b border-[hsl(var(--border))] px-4 py-3 flex items-center justify-between">
          ...
          <a
            href="/downloads"
            className="shrink-0 rounded-lg bg-[hsl(var(--primary))] px-4 py-1.5 text-xs font-semibold text-white hover:opacity-90 transition-opacity"
          >
            {t('vpn_download_btn')}
          </a>
        </div>
      ...
      <div className="md:hidden flex border-b border-[hsl(var(--border))]">
        <button
          ...
          className={`flex-1 py-3 text-sm font-medium text-center transition-colors ${mobileView === 'list' ? 'text-[hsl(var(--primary))] border-b-2 border-[hsl(var(--primary))]' : 'text-[hsl(var(--muted-foreground))]'}`}
        >
          {t('vpn_title')}
        </button>
        <button
          onClick={() => setMobileView('detail')}
          className={`flex-1 py-3 text-sm font-medium text-center transition-colors ${mobileView === 'detail' ? 'text-[hsl(var(--primary))] border-b-2 border-[hsl(var(--primary))]' : 'text-[hsl(var(--muted-foreground))]'}`}
        >
          {t('vpn_how_to_use')}
        </button>
      </div>
      ...
      <div
        className={`${mobileView === 'detail' ? 'hidden md:flex' : 'flex'} w-full md:w-[320px] lg:w-[420px] border-r border-[hsl(var(--border))] flex-col shrink-0`}
      >
        <div className="p-4">
          <h1 className="text-lg font-semibold text-[hsl(var(--foreground))] mb-3">
            {t('vpn_title')}
          </h1>
            ...
            {stats.byProtocol.slice(0, 4).map(([proto, count]) => (
              <span key={proto} className="flex items-center gap-1">
                <span
                  ...
                  style={{ background: PROTOCOL_COLORS[proto] || '#888' }}
                />
                {proto} ({count})
              </span>
            ))}
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-4">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-10 gap-2">
              ...
              {loadingMsg && (
                <p className="text-xs text-[hsl(var(--muted-foreground))]">{loadingMsg}...</p>
              )}
            </div>
            ...
            filteredConfigs.map((config) => (
              ...
              >
                <span className="w-7 h-7 rounded-full overflow-hidden shrink-0 bg-white/10">
                  <img
                    src={flagUrl(config.countryCode)}
                    alt={config.country}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 text-xs text-[hsl(222,10%,50%)]">
                    <span
                      ...
                      style={{ background: PROTOCOL_COLORS[config.protocol] || '#888' }}
                    />
      ...
      <div
        className={`${mobileView === 'list' ? 'hidden md:flex' : 'flex'} flex-1 flex-col items-center justify-center py-6 md:py-10 px-4 md:px-8 relative`}
      >
        <div
          className="absolute inset-0 pointer-events-none"
          style={{
            background:
              'radial-gradient(circle at 50% 35%, rgba(139,92,246,0.04) 0%, transparent 60%)',
          }}
        />
          ...
          <div className="w-full max-w-lg space-y-6">
            <div className="flex items-center gap-3">
              <span className="w-10 h-10 rounded-full overflow-hidden border border-white/10">
                <img
                  src={flagUrl(selectedConfig.countryCode)}
                  alt={selectedConfig.country}
                  className="w-full h-full object-cover"
                />
              </span>
              <div>
                ...
                <div className="flex items-center gap-2 text-xs text-[hsl(var(--muted-foreground))]">
                  <span
                    className="w-2 h-2 rounded-full"
                    style={{ background: PROTOCOL_COLORS[selectedConfig.protocol] || '#888' }}
                  />
                  <span className="uppercase font-medium">{selectedConfig.protocol}</span>
                  <span className="opacity-50">|</span>
                  <span>
                    {selectedConfig.listType === 'black'
                      ? t('vpn_black_list')
                      : t('vpn_white_list')}
                  </span>
                  <span className="opacity-50">|</span>
            ...
            {showFullUri === selectedConfig.id && fullUri && (
              <div className="bg-[hsl(var(--muted))] rounded-xl p-4">
                ...
                <pre className="text-xs text-[hsl(var(--muted-foreground))] break-all whitespace-pre-wrap font-mono max-h-40 overflow-y-auto">
                  {fullUri}
                </pre>
              </div>
            )}
            ...
            <div className="bg-[hsl(var(--muted))] rounded-xl p-4 space-y-2">
              ...
              <div className="mt-3 pt-3 border-t border-[hsl(var(--border))]">
                <p className="text-xs text-[hsl(var(--muted-foreground))]">
                  <span className="font-medium">{t('vpn_black_list')}</span> —{' '}
                  {t('vpn_black_explain')}
                </p>
                <p className="text-xs text-[hsl(var(--muted-foreground))] mt-1">
                  <span className="font-medium">{t('vpn_white_list')}</span> —{' '}
                  {t('vpn_white_explain')}
                </p>
              </div>
            </div>
