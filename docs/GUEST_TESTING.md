...
Откройте <http://localhost:3001/demo> и нажмите «Продолжить без регистрации». Откроется приложение `/app` с импортированными серверами; кабинет подписки доступен по `/dashboard`. Подробности экспорта конфигураций: [IMPORTED_SERVERS.md](IMPORTED_SERVERS.md). API использует порт 3100; если он уже запущен в этой задаче, второй экземпляр запускать не нужно. Веб-сервер слушает только loopback.
