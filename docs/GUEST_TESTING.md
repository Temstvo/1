...
- Статическая вёрстка главной, гостевого входа и login проверена офлайн в Chromium на 1440, 768, 390 и 320 px: шрифт загружается, горизонтального переполнения нет, FAQ раскрывается. Скриншоты и результаты: `.local/ui/`; команда `node scripts/preview-static.cjs` после сборки.
- Полная браузерная проверка работающего сервиса не выполнена: автоматическая проверка разрешений заблокировала запуск веб-сервера без объяснения конкретной причины. Офлайн-проверка отключает JavaScript приложения, не проверяет вкладки React и запросы из браузера к API. Внешние YooKassa/Marzban не проверены с реальными ключами.
